@echo off
REM A script to remotely power off a XBox One from a Windows PC via a Linux device's IR device, from https://github.com/Vegz78/WakeXBOne
...
SET "REMPATHCOMMAND=/../../_PowerOffXBOne.sh"	&REM The remote path and name of the script file to execute remotely
...
:EXITSCRIPT
ECHO XBox One still responding, run again?...
CALL :EXITING
EXIT /B 1

:FINISHSCRIPT
ECHO XBox One is already off/not responding to ping!...
CALL :EXITING
EXIT /B 0
